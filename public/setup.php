...
MMMM;
	...
	private function _generateRowClass($name)
	{
		...
		$class = "<?php\nabstract class Application_Model_Db_Row_" . $this->toCamelCase($name) . " extends Application_Model_BaseItem\n{\n";
		$tableName = substr($name, -1)== 'y' ? $this->toCamelCase(substr($name, 0, strlen($name)-1))."ie" : $this->toCamelCase($name);
		$class .= "\tprotected \$_table = 'Application_Model_".$tableName."s';\n";
	...
	private function _generateTableClass($name)
	{
		...
		$class = <<<CLSS
<?php
abstract class Application_Model_Db_Table_{$this->toCamelCase($tableName)}s extends Application_Model_BaseCollection
{
...
CLSS;
	...
	private function _generateTableModelClass($name)
	{
		...
		$class = <<<CLSS
<?php
class Application_Model_{$this->toCamelCase($tableName)}s extends Application_Model_Db_Table_{$this->toCamelCase($name)}s
{
