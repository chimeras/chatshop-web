...
class Facebook
{
	...
	$this->setAppId($config->get('appId'));
	$this->setApiSecret($config->get('secret'));
	if ($config->get('cookie') != NULL) {
	    $this->setCookieSupport($config->get('cookie'));
	}
	if ($config->get('domain') != NULL) {
	    $this->setBaseDomain($config->get('domain'));
	}
	if ($config->get('fileUpload') != NULL) {
	    $this->setFileUploadSupport($config->get('fileUpload'));
	}
    }
	...
	if (!$this->sessionLoaded) {
		...
		if (isset($_COOKIE[$cookieName])) {
		    ...
		    $session = $this->validateSessionObject($sessionObj);
...
//    echo $currentUrl;
//    echo str_replace("login", "loginfbc/", $currentUrl)."?login=facebook_popup";
//    exit;
